__global__ void oddStep(int *input, int *output, int N) {
...
__global__ void evenStep(int *input, int *output, int N) {
...
int main(void) {
	...
	for (int iteration = 0; iteration < K ; iteration++) {
		oddStep <<<N/THREADS_PER_BLOCK , THREADS_PER_BLOCK >>> (d_input, d_ouput, N);

		evenStep<<<N/THREADS_PER_BLOCK , THREADS_PER_BLOCK >>> (d_ouput, d_input, N);
